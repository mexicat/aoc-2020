defmodule AdventOfCode.Day04Test do
  ...
  test "part2" do
    ...
    assert not valid_field?("byr", "2003")

    assert valid_field?("hgt", "60in")
    assert valid_field?("hgt", "190cm")
    assert not valid_field?("hgt", "190in")
    assert not valid_field?("hgt", "190")

    assert valid_field?("hcl", "#123abc")
    assert not valid_field?("hcl", "#123abz")
    assert not valid_field?("hcl", "123abc")

    assert valid_field?("ecl", "brn")
    assert not valid_field?("ecl", "wat")

    assert valid_field?("pid", "000000001")
    assert not valid_field?("pid", "0123456789")
  end
end
